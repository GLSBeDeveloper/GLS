...
exports.m_optimize = async (req, res, next) => {
    ...
    const problem = req.problem;
    console.log('TCL: exports.m_optimize -> req.problem', req.problem._id);
    ...
    const childProcess = await ProblemUtils.optimize(problem, options, _logger, (err, solution) => {
        console.log('TCL: exports.m_optimize -> problem', problem._id);
        // end handler
        try {
            if (err) {
                throw err;
    ...
    async function _saveSolutionToDb(solution) {
        ...
        console.log('TCL: asyncfunction_saveSolutionToDb -> _query', _query);
        try {
            // remove old solution
