...
exports.m_checkDuplicates = async (req, res, next) => {
    log.debug('m_checkDuplicates', req.user.username);

    const table = req.params.table;
    const ids = vrpUtils.toArray(_.get(req.body, 'ids'));

    // list of tables that can check if has duplicates
    const allowedTables = ['Item', 'DeliveryMaster', 'Vehicle', 'VehicleType'];

    try {
        if (_.isEmpty(ids)) {
            throw new Error('Parameter `ids` is required.');
        } else if (_.indexOf(allowedTables, table) < 0) {
            throw new Error(`Invalid table (case-sensitive). Only these tables are allowed: ${_.join(allowedTables, ', ')}.`);
        }
