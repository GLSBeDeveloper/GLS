...
exports.m_getAll = async (req, res, next) => {
    log.debug('m_getAllVehicleLocationLog', req.user.username);
    ...
    try {
        ...
        const result = await colVehicleLocationLog.find(where, {
            projection: { _id: 0 },
        }).toArray();
...
async function createVehicleLocationLog(req, logs) {
    ...
    function isVehicleLogValid(vehicleLog) {
        if (_.isEmpty(vehicleLog)) {
            ...
            vehicleLog.RecordedTime = moment(vehicleLog.RecordedTime).toDate();
