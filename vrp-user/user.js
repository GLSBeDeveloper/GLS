...
exports.m_createUser = async (req, res, next) => {
    ...
    try {
        ...
        const fields = ['fullname', 'username', 'password', 'usergroup', 'email', 'role', 'created_date', 'modified_date', 'nric', 'phone', 'note'];
        await colUser.insertOne(_.pick(user, fields));

        // notify user of new password
        _notifyPasswordToUser({ user: _.pick(user, ['fullname', 'email']), plainText });
    ...
    try {
        ...
        const promises = _.map(usernames, async (username) => {
            ...
            const updateResult = await colUser.findOneAndUpdate({
                ...
                projection: { _id: 0, email: 1, fullname: 1 },
                upsert: false,
            });
 ...
 * @returns {void}
 */
function _notifyPasswordToUser(result) {
    _.each(_.concat(result), (credentials) => {
        try {
