...
exports.setup = vrpUtils.setupOnce(this, async (passport, mongoDb) => {
    ...
    schedule.scheduleJob('23 59 * * *', () => { lockedAccess = []; });
});
...
exports.m_isAuthNotDriver = [m_isLoggedIn, m_isRole([vrpEnum.UserRole.ADMIN, vrpEnum.UserRole.PLANNER, vrpEnum.UserRole.DEFAULT])];
...
exports.m_userFilter = (req, res, next) => {
    ...
    if (!vrpUserUtils.isAdmin(req.user)) {
        ...
        Object.assign(req.filter, { username: { $elemMatch: { $in: [vrpMongo.toCaseInsensitiveRegex(username)] } } });
    }
    next();
};
...
exports.m_userGroupFilter = (req, res, next) => {
    ...
    if (usergroup) {
        Object.assign(req.filter, { usergroup: usergroup.toUpperCase() });
    }
    next();
};
...
exports.m_returnUserInfo = async (req, res, next) => {
    ...
    const response = Object.assign({ token: _.get(req, 'user.token') }, req.user);
...
function m_login(req, res, next) {
    ...
    return vrpPassportUtils.login(async (err, user) => {
        try {
            ...
            } else if (vrpUserUtils.isPlanner(user) || vrpUserUtils.isDriver(user)) {
    ...
    if (account && account.nFailedLogin > 0) {
        if (account.locked) {
            ...
            if (waitingTimeLeft > 0) {
                ...
                _.remove(lockedAccess, { id }); // remove from locked list
                next();
            }
        } else {
            next();
...
function _lockIfFailedAttemptsReachLimit(req) {
    ...
    if (!account) {
        account = { id, nFailedLogin: 1, failedLoginAt: new Date(), locked: false };
...
async function _checkIfLoggedInElsewhere(userName, currentSessionId) {
    try {
        const selector = { session: { $regex: `"username":"${userName}"`, $options: 'i' } };
        if (currentSessionId) {
            selector._id = { $ne: currentSessionId };
        }
